...
class PasswordController extends BaseController {
	...
	{	
		...
		if ($credentials = array('email' => Input::get('email'), 'password' => Input::get('password'))) {
			...
			$check = DB::table('users_fste')->where('email', $credentials['email'])->first();
	...
	public function reset($token)
	{
		...
		if (count($email) >= 1 ) {
			...
			return View::make('users.password_reset_link', compact('token', 'email'));
		} 

		else {
			return View::make('users.password_reset_error');
		}
	...
	public function update()
	{
		...
		$check = DB::table('password_reminders_fste')->where('token', $inputs['token'])->where('email', $inputs['email'])->first();
		...
		if ( count($check) == 1 ) {
			...
			DB::table('password_reminders_fste')->where('email', $inputs['email'])->delete();

			$path = Session::get('language');

			return Redirect::route('users.login')->withSuccess(Lang::get($path.'.reset_success'));
