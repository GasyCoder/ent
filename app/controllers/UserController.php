...
class UserController extends BaseController {
	...
	public function users_data() {	
		...
		if (!empty($text_query)) {
			...
			if(Input::has('q')) {
				$students = DataUsers::where('fullname', 'like', '%' .$text_query. '%')->orWhere('registration_num', $text_query);
			}

			$students = $students->paginate(300);

			return View::make('admin.users_data', compact('students'), [ 'students' => $students->appends(Input::except('page')) ]);
		} 

		else {
			
			$students = DataUsers::orderBy('fullname', 'asc')->paginate(300);
			return View::make('admin.users_data', compact('students'));
		}


	}

	public function users_data_export() {
	    ...
	    fputcsv($handle, array('fullname', 'class', 'registration_num', 'parcour', 'mention', 'admission', 'gender', 'birthday', 'birth_localite', 'region', 'address', 'email', 'phone'), $delimiter=';');

	    foreach($table as $row) {
	        fputcsv($handle, array($row['fullname'], $row['class'], $row['registration_num'], $row['parcour'], $row['mention'], $row['admission'], $row['gender'], $row['birthday'], $row['birth_localite'], $row['region'], $row['address'], $row['email'], $row['phone']), $delimiter=';');
	    }
				...
				if (Input::hasFile('csv_file')) {
					...
					if ($extension == "csv" OR $extension == "CSV") {
						...
						function csv_to_array($filename='', $delimiter=';')
	                    {
			         	...
			         	DB::table('users_data_fste')->insert($areas);	
			...
			if ($validation->fails()) {
				...
				if (Input::hasFile('image')) {
					...
					$user = User::create([
						...
						'birth_localite' => e(Input::get('birthday')),
